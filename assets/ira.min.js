...
ira.stickyHeader=function(){var CLASS_NAME='sticky-header';var stickyHeaderEnabled=;var notificationBarEnabled=;if(window.location.pathname.indexOf("products")!==-1&&ira.cache.$html.hasClass('supports-touch')){return $('.'+CLASS_NAME).removeClass(CLASS_NAME);}
...
ira.transparentHeader=function(){var transparentHeaderEnabled=;var stickyHeaderEnabled=;var className='transparent-header';if(window.location.pathname!=='/'){$('.'+className).removeClass(className);}
...
var transparentHeaderEnabled=;var $nodes={open:$(config.openClassSelector),close:$(config.closeClassSelector),searchBar:$(config.searchBarSelector),input:$(config.searchInputSelector)};$nodes.close.add($nodes.input).attr('tabindex',-1);function drawerOpenHandler(e){$nodes.searchBar.addClass(config.showSearchClass);$nodes.close.add($nodes.input).attr('tabindex',0)
if(!((navigator.userAgent.match(/(iPod|iPhone|iPad)/)&&)||navigator.appVersion.indexOf("MSIE 8")>0)){$nodes.input[0].select();}
...
Carousel.prototype.load=function(){var enableKenBurns=;if(!this.isPrimary())enableKenBurns=;if(enableKenBurns)this.loadKenBurns();this.loadSlickCarousel();this.repositionSlickDots();if(this.isPrimary()){this.fullScreen();this.pauseUntilFirstImageLoads();this.positionText();}};Carousel.prototype.positionText=function(){var carouselOverlay=this.$carousel.find('.carousel-overlay');setPaddingTop();$(window).on('resize',setPaddingTop);function setPaddingTop(){var headerHeight=$('.site-header').outerHeight();carouselOverlay.each(function(){$(this).css('padding-top',headerHeight);});}};Carousel.prototype.loadKenBurns=function(){var carouselImages=this.$carousel.find('.carousel__image');if(carouselImages.length==1){this.$carousel.on('init beforeChange',function(){this.kenBurnsLoopSingleImage(carouselImages);}.bind(this))}
else{this.kenBurnsEventListeners();}};Carousel.prototype.loadSlickCarousel=function(){this.$carousel.slick({autoplay:true,dots:true,arrows:false,mobileFirst:true,respondTo:'min',adaptiveHeight:false,autoplaySpeed:this.config.timePerSlide,pauseOnHover:false,draggable:false,fade:true,lazyload:'progressive',});};Carousel.prototype.isProgressBarEnabled=function(){var isEnabled;if(this.isPrimary()){isEnabled=;}
else{isEnabled=}
return isEnabled;}
